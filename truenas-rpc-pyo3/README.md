...
The optional embedded-CPython bridge that runs `truenas-rpc` `python:true` method bodies in
...
`truenas-rpc`, `serde_json`, and `pyo3-ffi` — the raw CPython C-API bindings plus libpython
...
The core's python *pipeline* is covered by a mock `PyDispatcher` in `truenas-rpc`'s tests;
this crate is excluded from the line-coverage gate and tested with
`cargo test -p truenas-rpc-pyo3` (needs libpython + `msgspec`).
