        ...
        match kind_of(m) {
            Kind::Plain => {
                ...
                methods.push_str(&format!(
                    "    /// Handler for `{}`.\n    fn {}(&self, request: {params}, cx: &truenas_jsonrpc::RequestCtx<S>) -> Result<{result}, truenas_jsonrpc::JsonRpcError>;\n",
        ...
        match kind_of(m) {
            Kind::Plain => {
                ...
                body.push_str(&format!(
                    "    let h = handlers.clone();\n    let builder = builder.method(truenas_jsonrpc::JsonRpcMethod::new({def}, move |request: {params}, cx: &truenas_jsonrpc::RequestCtx<S>| h.{}(request, cx)))?;\n",
                    m.handler
                ));
            }
            Kind::Filterable => {
                ...
                body.push_str(&format!(
                    "    let h = handlers.clone();\n    let builder = builder.filterable(truenas_jsonrpc::FilterableJsonRpcMethod::<{params}, {entry}, _>::new({def}, move |request: {params}, cx: &truenas_jsonrpc::RequestCtx<S>, filters: &truenas_jsonrpc::CompiledFilters, options: &truenas_jsonrpc::CompiledOptions| h.{}(request, cx, filters, options)))?;\n",
                    m.handler
                ));
            }
            Kind::Subscription => {
                ...
                body.push_str(&format!(
                    "    let builder = builder.subscription(truenas_jsonrpc::SubscriptionDef::<{params}, {notifies}>::new({def}))?;\n"
                ));
            }
            Kind::Python => {
...
fn method_def_expr(spec: &Spec, wire: &str, m: &MethodSpec) -> String {
    let mut s = format!("truenas_jsonrpc::MethodDef::new({})", str_lit(wire));
