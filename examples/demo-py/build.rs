...
fn main() {
    ...
    build().emit_types().expect("json-idl -> types codegen failed");
    build().emit_client().expect("json-idl -> client codegen failed");
    build().emit_pyclient().expect("json-idl -> pyclient codegen failed");
}
