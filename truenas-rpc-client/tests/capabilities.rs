...
fn auth_proto() -> JsonRpcProtocol<()> {
    JsonRpcProtocol::<()>::builder("auth", "1")
        ...
        .method(RpcMethod::new(
            MethodDef::new("secret"),
            |_a: Empty, _cx: &RequestCtx<()>| Ok::<_, JsonRpcError>(json!({ "token": "s3cr3t" })),
        ))
        .unwrap()
        .build()
}
...
) -> (std::path::PathBuf, TruenasRpcServer<()>) {
    let path = std::env::temp_dir().join(format!("tnrpc-cap-{tag}-{}.sock", std::process::id()));
    let _ = std::fs::remove_file(&path);
    let srv = TruenasRpcServer::<()>::builder("cap-server")
        .protocol(name, proto)
        .build();
...
async fn authenticate_gates_the_call_then_graceful_close() {
    ...
    assert!(
        before.is_err(),
        "gated method must be refused before auth: {before:?}"
    );
    ...
    let after = CallEngine::call(&client, MethodKey::Name("secret"), b"{}")
        .await
        .unwrap();
    assert_eq!(
        serde_json::from_slice::<Value>(&after).unwrap()["token"],
        "s3cr3t"
    );
...
fn sub_proto() -> JsonRpcProtocol<()> {
    JsonRpcProtocol::<()>::builder("sub", "1")
        .subscription(SubscriptionDef::<Empty, Event>::new(MethodDef::new(
            "events",
        )))
        .unwrap()
        .build()
...
async fn subscribe_then_receive_a_published_notification() {
    ...
    let ack = CallEngine::call(&client, MethodKey::Name("events"), b"{}")
        .await
        .unwrap();
    ...
    assert!(
        after.is_err(),
        "no notification should arrive after unsubscribe, got {after:?}"
    );
...
async fn dropping_a_call_cancels_it_server_side() {
    ...
    let c = client.clone();
    let handle =
        tokio::spawn(async move { CallEngine::call(&*c, MethodKey::Name("slow"), b"{}").await });
    ...
    assert!(
        ...
        "dropping the call must cancel the handler server-side"
    );
    let _ = std::fs::remove_file(&path);
}
...
async fn call_with_progress_streams_updates_then_the_reply() {
    ...
    assert_eq!(
        updates.len(),
        3,
        "three progress updates, all before the reply"
    );
