//! Rust↔Zig filter A/B — a line-for-line mirror of `zig/bench/filter_bench.zig`: the same
//! `Entry`, the same dataset generation, the same `x.query` wires, and the same
//! WARMUP/TRIALS/ITERS protocol (min-of-trials). It measures the Rust filter engine
//! (predicates over a `serde_json::Value` *view* of each row) against the Zig engine
//! (predicates on the typed struct via a comptime field accessor).
//!
...
//!     cargo run --release --example zig_filter_ab
...
async fn main() {
    // Same logical rows as filter_bench.zig; leaked so the handler can yield `&Entry`.
