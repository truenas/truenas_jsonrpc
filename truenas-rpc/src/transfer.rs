...
//! helpers (`sendfile`/`splice`/`SCM_RIGHTS`) live in `truenas-rpc-server`, where the
...
/// `SCM_RIGHTS` helpers built on the fd) lives in `truenas-rpc-server`. Keeping the trait
