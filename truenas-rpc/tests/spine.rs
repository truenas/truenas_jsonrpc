...
use truenas_rpc::{
    AuditOutcome, Dispatched, JsonRpcError, JsonRpcProtocol, MethodDef, NullOutbound, Outbound,
    RequestCtx, RequestInfo, Roles, RpcMethod, Session, SessionLifecycle,
};
...
fn echo_proto() -> JsonRpcProtocol<()> {
    JsonRpcProtocol::<()>::builder("test", "1.0.0")
        .method(RpcMethod::new(
            MethodDef::new("echo"),
            |a: EchoArgs, _cx: &RequestCtx<()>| Ok(EchoResult { echo: a.msg }),
        ))
        .unwrap()
        .method(RpcMethod::new(
            MethodDef::new("boom"),
            |_a: EchoArgs, _cx: &RequestCtx<()>| {
...
async fn missing_required_param_is_invalid_params() {
    let proto = echo_proto();
    let s = session(&proto, Some(()));
    let resp = call(&proto, &s, &req("echo", Some(json!({})), Some(ID)))
        .await
        .unwrap();
...
async fn handler_error_passthrough() {
    ...
    let resp = call(
        &proto,
        &s,
        &req("boom", Some(json!({"msg": "x"})), Some(ID)),
    )
    .await
    .unwrap();
...
async fn unknown_method_is_method_not_found() {
    let proto = echo_proto();
    let s = session(&proto, Some(()));
    let resp = call(&proto, &s, &req("nope", Some(json!({})), Some(ID)))
        .await
        .unwrap();
...
async fn parse_and_structural_errors() {
    ...
    let resp = call(
        &proto,
        &s,
        &req("echo", Some(json!({"msg": "x"})), Some("not-a-uuid")),
    )
    .await
    .unwrap();
    assert_eq!(resp["error"]["code"], -32600);
...
async fn invalid_params_precedes_not_authorized() {
    ...
    let resp = call(
        &proto,
        &s,
        &req("echo", Some(json!({"msg": "x"})), Some(ID)),
    )
    .await
    .unwrap();
...
async fn async_method_happy_path() {
    ...
    let resp = call(
        &proto,
        &s,
        &req("aecho", Some(json!({"msg": "yo"})), Some(ID)),
    )
    .await
    .unwrap();
    assert_eq!(
        resp,
        json!({"jsonrpc": "2.0", "result": {"echo": "yo"}, "id": ID})
    );
}
...
async fn session_gate_and_setup() {
    ...
    let proto = JsonRpcProtocol::<String>::builder("test", "1.0.0")
        .method(RpcMethod::new(
            ...
            |_a: EchoArgs, cx: &RequestCtx<String>| {
                let who = cx
                    .session()
    ...
    let resp = call(
        &proto,
        &s,
        &req("whoami", Some(json!({"msg": "_"})), Some(ID)),
    )
    .await
    .unwrap();
    assert_eq!(
        resp["error"]["code"], -32002,
        "expected SESSION_NOT_ESTABLISHED"
    );
    ...
    let resp = call(
        &proto,
        &s,
        &req("$/sessionSetup", Some(json!({"token": "bad"})), Some(ID)),
    )
    .await
    .unwrap();
    assert_eq!(resp["error"]["code"], -32000);
    ...
    let resp = call(
        &proto,
        &s,
        &req("$/sessionSetup", Some(json!({"token": "good"})), Some(ID)),
    )
    .await
    .unwrap();
...
async fn server_info_probe() {
    ...
    assert_eq!(
        resp["result"],
        json!({"name": "truenas", "version": "25.10"})
    );
}
...
async fn audit_redacts_secret_fields() {
    ...
    let proto = JsonRpcProtocol::<()>::builder("test", "1.0.0")
        ...
        .audit_sink(
            move |r: &RequestInfo,
                ...
                sink.lock()
                    .unwrap()
                    .push((r.params.clone(), msg.map(str::to_string)));
            },
        )
        .build();
    let s = session(&proto, Some(()));
    let resp = call(
        ...
        &req(
            "login",
            Some(json!({"user": "u", "password": "hunter2"})),
            Some(ID),
        ),
    )
    .await
...
async fn progress_reaches_the_outbound_sink() {
    ...
    let resp = call(
        &proto,
        &s,
        &req("work", Some(json!({"msg": "_"})), Some(ID)),
    )
    .await
    .unwrap();
    ...
    assert_eq!(
        msgs[0],
        json!({"jsonrpc": "2.0", "method": "$/progress", "params": {"id": ID, "percent": 50.0, "description": "half"}})
    );
}
...
async fn cancel_and_close_control_ops_are_audited() {
    ...
    let proto = JsonRpcProtocol::<()>::builder("test", "1.0.0")
        ...
        .audit_sink(
            move |r: &RequestInfo,
                ...
                sink.lock()
                    .unwrap()
                    .push((r.method.clone(), outcome.succeeded()));
            },
        )
        .build();
    let s = session(&proto, Some(()));
    ...
    let rows = captured.lock().unwrap();
    assert!(
        rows.iter().any(|(m, ok)| m == "$/sessionSetup" && *ok),
        "setup audited"
    );
    assert!(
        rows.iter().any(|(m, ok)| m == "$/cancelRequest" && !*ok),
        "cancel-denial audited"
    );
    assert!(
        rows.iter().any(|(m, ok)| m == "$/sessionClose" && *ok),
        "close audited"
    );
}
